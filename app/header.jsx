...
export default function Header({ videoRef, setFilter, filter, content }) {
	...
	const handleScroll = (e) => {
		...
		if (carouselContainer) {
			const position = carouselContainer.scrollTop;
			...
			let marginTop = mapClamped(
				...
				0,
				window.innerHeight,
				(window.innerHeight - logo.current.offsetHeight) / 2,
				0
			);
			logo.current.style.padding = `clamp(${size / 100}rem, 1rem, 2rem)`;
			logo.current.style.paddingLeft = `clamp(${size / 100}rem, 3rem, 6rem)`;
	...
	useEffect(() => {
		...
		// Also listen to wheel events for immediate feedback
		const handleWheel = (e) => {
			handleScroll();
		};
	...
	return (
		<header className="header">
			...
			<div className="header-content-two"></div>
		</header>
	);
}
